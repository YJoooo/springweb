...
@Controller
@SessionAttributes("member") // spring에서 session 설정
public class A04_SessionCtrl {
	...
	public String login88(@ModelAttribute("member") Member m, Model d) {
		...
		if(m.getId().equals("himan")&& m.getPass().equals("8888")) {
			m.setName("홍길동");
			m.setAuth("관리자");
			m.setPoint(10000);
			page = "a12_main.jsp";
	...
	public String callSession03(HttpServletRequest request) {
		HttpSession session = request.getSession();
		session.setAttribute("prod",new Product("커피",5000,3));
		return"WEB-INF\\views\\a01_basic\\a10_session03.jsp";
	}
	@RequestMapping("callSession04.do")
	public String callSession04() {
